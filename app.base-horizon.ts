...
async function readBlockSummary() {
...
function mount() {
  ...
  subtitle.textContent =
    "Wallet connection, Base chain validation, and read-only horizon view of onchain state.";
  ...
  btnBlock.textContent = "Block Summary";
  ...
  btnBlock.onclick = async () => {
    try {
      render(["Reading block summary…"]);
      const s = await readBlockSummary();
      render([
        "Block Summary",
        `Network: ${active.label}`,
